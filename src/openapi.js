...
const FIGURES = {
  ...
  consumption_lifecycle:
    "Both adjustments. The most modelled of the four and the one to report. Countries only.",
};
...
const COMMON_ERRORS = { 404: { $ref: "#/components/responses/NotFound" }, 429: { $ref: "#/components/responses/RateLimited" } };
...
export function buildSpec({ version = PKG.version } = {}) {
  ...
  return {
    ...
    tags: [
      ...
      { name: "zone", description: `Bidding zones and balancing regions, for the ${Object.keys(ZONES).length} countries that publish below national level.` },
      { name: "bulk", description: "Every country in one request." },
    ],
    paths: {
      "/v2/{code}/yearly": op({
        summary: "Annual average for one country",
        description: "Available for every country. Rewritten at most weekly, so `generated_at` moves rarely — correct for a figure that changes once a year.",
        tags: ["country"], params: ["code"], ref: "YearlyReading",
        okText: "The annual average.",
      }),
      "/v2/{code}/past-hour": op({
        summary: "Last completed clock hour",
        description: "The newest hour holding all of its points. Immutable once published. Absent (404) until a complete hour exists in the provider's window.",
        tags: ["country"], params: ["code"], ref: "HourReading",
        okText: "A complete hour. `complete` is always true.",
      }),
      "/v2/{code}/current-hour": op({
        summary: "Hour in progress",
        description: "The newest hour with any data. Changes between runs as the rest of the hour arrives, so `complete` is usually false and `period_end` is in the future.",
        tags: ["country"], params: ["code"], ref: "HourReading",
        okText: "The hour in progress.",
      }),
      "/v2/{code}/history/{date}": op({
        summary: "One UTC day of hourly means",
        description: "Retained for 365 days. A day whose date is in the past will never be rewritten and is safe to cache indefinitely.",
        tags: ["country"], params: ["code", "date"], ref: "HistoryDay",
        okText: "One day, columnar.",
      }),
      "/v2/{code}/{zone}/past-hour": op({
        summary: "Last completed clock hour for a zone",
        description: "Zone documents omit both consumption figures: the import adjustment is a national figure and one bidding zone's import mix is not the country's.",
        tags: ["zone"], params: ["code", "zone"], ref: "HourReading",
        okText: "A complete hour for the zone.",
      }),
      "/v2/{code}/{zone}/current-hour": op({
        summary: "Hour in progress for a zone",
        description: "A zone whose provider failed this run is absent until the next one — treat a zone 404 as *ask the country instead*.",
        tags: ["zone"], params: ["code", "zone"], ref: "HourReading",
      ...
      "/v2/{code}/{zone}/history/{date}": op({
        ...
        tags: ["zone"], params: ["code", "zone", "date"], ref: "HistoryDay",
